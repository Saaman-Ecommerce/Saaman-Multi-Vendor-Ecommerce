...
class PriceConverter {
  static String convertPrice(BuildContext context, double price,
    ...
    return '${Provider.of<SplashProvider>(context, listen: false).myCurrency.symbol}${(_singleCurrency ? price : price * Provider.of<SplashProvider>(context, listen: false).myCurrency.exchangeRate * (1 / Provider.of<SplashProvider>(context, listen: false).usdCurrency.exchangeRate)).toStringAsFixed(2).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}';
  }
  ...
  static String convertPriceWithoutSymbol(BuildContext context, double price,
    ...
    return '${(_singleCurrency ? price : price * Provider.of<SplashProvider>(context, listen: false).myCurrency.exchangeRate * (1 / Provider.of<SplashProvider>(context, listen: false).usdCurrency.exchangeRate)).toStringAsFixed(2)}';
  }
  ...
  static double convertAmount(double amount, BuildContext context) {
    return double.parse(
        '${(amount * Provider.of<SplashProvider>(context, listen: false).myCurrency.exchangeRate * (1 / Provider.of<SplashProvider>(context, listen: false).usdCurrency.exchangeRate)).toStringAsFixed(2)}');
  }
