...
import 'package:Saaman_Vendor/data/model/response/category_wise_shipping_model.dart';
import 'package:Saaman_Vendor/data/model/response/shipping_model.dart';
...
class ShippingProvider extends ChangeNotifier {
  ...
  Future<void> deleteShipping(BuildContext context, int id) async {
    ...
    if (response.response.statusCode == 200) {
      ...
      showCustomSnackBar(
          getTranslated('shipping_method_deleted_successfully', context),
          context,
          isError: false);
      getShippingList(context,
  ...
  Future<void> setShippingMethodType(BuildContext context, String type) async {
    ...
    if (apiResponse.response != null &&
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(
            getTranslated('shipping_method_updated_successfully', context)),
  ...
  Future<ApiResponse> setCategoryWiseShippingCost(BuildContext context,
    ...
    print(
        '=====Update category cost ====>${_ids.toString()}===>${cost.toString()}===>${multiPly.toString()}');
    if (apiResponse.response != null &&
        apiResponse.response.statusCode == 200) {
      getCategoryWiseShippingMethod(context);
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content:
            Text(getTranslated('category_cost_updated_successfully', context)),
        backgroundColor: Colors.green,
      ));
    } else {
      ApiChecker.checkApi(context, apiResponse);
    ...
    for (int i = 0; i < categoryWiseShipping.length; i++) {
      categoryWiseShipping.forEach((categoryWiseShipping) {
        _shippingCostController.add(TextEditingController(
            text: categoryWiseShipping.cost.toString() ?? 0.0));
  ...
  Future<ApiResponse> assignThirdPartyDeliveryMan(
      BuildContext context, String name, String trackingId, int orderId) async {
    _isLoading = true;
    notifyListeners();
    ApiResponse apiResponse = await shippingRepo.assignThirdPartyDeliveryMan(
        name, trackingId, orderId);

    print(
        '=====Set third party shipping ====>${name.toString()}===>${trackingId.toString()}===>${orderId.toString()}');
    if (apiResponse.response != null &&
        apiResponse.response.statusCode == 200) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(
